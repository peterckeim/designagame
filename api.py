...
NUMBER_RESULTS_REQUEST = endpoints.ResourceContainer(num_results=messages.IntegerField(1),)
...
class HangmanApi(remote.Service):
    ...
    def create_user(self, request):
        ...
        user = User(name=request.user_name, email=request.email, games_played=0, career_points=0, performance=0)
        user.put()
    ...
    def new_game(self, request):
        ...
        #game.key = ndb.Key(Game, game.id, parent=user.key)
        game.put()
    ...
    def make_move(self, request):
        ...
        if len(request.guess) > 1 or request.guess not in "abcdefghijklmnopqrstuvwxyz":
        ...
        if request.guess in game.target_string:
            ...
            game.history.append({"guess":request.guess,"message":msg,"strikes":game.strikes_remaining})
        else:
        ...
        for i in range(len(game.target_string)):
            if game.target_string[i] in game.correct_letters: 
                game.shown_string = game.shown_string[:i] + game.target_string[i] + game.shown_string[i+1:]

        if game.shown_string == game.target_string:
            game.end_game(True)
            theUser.games_played += 1
            theUser.career_points += game.strikes_remaining
            theUser.performance = theUser.career_points / theUser.games_played
            theUser.put()
    ...
    def get_average_strikes(self, request):
        ...
        return StringMessage(message=memcache.get(MEMCACHE_STRIKES_REMAINING) or '')
    ...
    def _cache_average_strikes():
        ...
        if games:
            ...
            memcache.set(MEMCACHE_STRIKES_REMAINING,
                         'The average strikes remaining is {:.2f}'.format(average))
    ...
    def get_high_scores(self, request):
        ...
        if request.num_results:
		    return ScoreForms(items=[score.to_form() for score in qu.fetch(limit=request.num_results)])
    ...
    def get_user_rankings(self, request):
        """generates ranked list (leaderboard) of users based on user performance, then by career points,
        ...
        if request.num_results:
		    return UserForms(items=[user.to_form() for user in qu.fetch(limit=request.num_results)])
